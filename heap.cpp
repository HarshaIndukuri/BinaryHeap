...
using namespace std;
	...
	class BinaryHeap{
		...
		void insert(int value){
			...
			if (csize>=msize) cout<<"Heap overflow!"<<endl;
			else{
				csize++;
				arr[csize-1]=value;
		...
		void sort(int a){
			if(lchild(a)>csize-1 || (arr[lchild(a)]>arr[a] && arr[rchild(a)]>arr[a])) return;

			else if(lchild(a)==csize-1) swap(arr+a,arr+lchild(a));		
